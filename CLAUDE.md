...
Status: domain layer, Prisma/Postgres infrastructure, credentials auth, and the main `/` page are fully wired end-to-end (Phases 0–5). Magic-link sign-in and the admin back-office are not yet built — check `src/` before assuming a layer exists.
...
- `src/application/` — use cases (`log-task`, `remove-today-log`, `list-today-tasks` implemented; admin `list-accounts` / `list-tasks-with-stats` not yet) that orchestrate domain objects via the repository interfaces and return plain DTOs (never raw entities — entities have private constructors and aren't meaningful to serialize across the Server Action boundary). Called from `src/app/actions.ts` and `src/app/page.tsx`, which resolve `userId`/`timezone` from `auth()` — use cases themselves take those as params rather than reading the session, so they stay framework-free and unit-testable with the `tests/support/in-memory-repositories.ts` fakes.
