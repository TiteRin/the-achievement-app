...
function formatDayLabel(day: string): string {
  ...
  // `day` is already the correct calendar date, resolved server-side.
...
function NavArrow({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      aria-label={direction === "previous" ? "Jour précédent" : "Jour suivant"}
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col gap-6 px-5 py-8">
      <header className="flex flex-col items-center gap-4">
        ...
        <DailyCounter count={dailyTotal} isToday={dayView.isToday} />
      </header>
      ...
      <div className="flex items-center gap-2">
        ...
        {dayView.nextDay && (
          <NavArrow direction="next" onClick={() => navigateDay("next")} disabled={isPending} />
        )}
      </div>
